...
class REDSDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.flow_root = Path(opt['dataroot_flow']) if opt['dataroot_flow'] is not None else None #dataroot_flow如果不在就化路径，如果在就不执行
        assert opt['num_frame'] % 2 == 1, (f'num_frame should be odd number, but got {opt["num_frame"]}')  #如果 opt['num_frame'] % 2 == 1是真，那么往下进行，反之，后面的输出。
        ...
        with open(opt['meta_info_file'], 'r') as fin: #打开meta_info_file文件路径并且处理异常
            for line in fin:
                ...
                self.keys.extend([f'{folder}/{i:08d}' for i in range(int(frame_num))]) #在Keys列表中增加

        # remove the video clips used in validation删除验证中正在使用的视频剪辑
        ...
        # temporal augmentation configs 时间增加配置
    ...
    def __getitem__(self, index):
        ...
        # determine the neighboring frames 确定相邻帧
        interval = random.choice(self.interval_list)

        # ensure not exceeding the borders  确保不要超出边界
        start_frame_idx = center_frame_idx - self.num_half_frames * interval
        end_frame_idx = center_frame_idx + self.num_half_frames * interval
        # each clip has 100 frames starting from 0 to 99    每个剪辑有100帧从0到99
        ...
        # get the neighboring LQ frames 获得低质量相邻帧
        img_lqs = []
        ...
        if self.flow_root is not None:
            ...
            # for random crop, here, img_flows and img_lqs have the same spatial size 对于随机剪裁，这里img_flows和img_lqs具有相同的空间大小
        ...
        # augmentation - flip, rotate 增强，旋转，翻转
...
@DATASET_REGISTRY.register()
class REDSDJPGataset(data.Dataset):
    """REDS dataset for deblur training.
    ...
    def __init__(self, opt):
        super(REDSDJPGataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq']) #转换成路径形式
        self.flow_root = Path(opt['dataroot_flow']) if opt['dataroot_flow'] is not None else None #dataroot_flow如果不在就化路径，如果在就不执行
        assert opt['num_frame'] % 2 == 1, (f'num_frame should be odd number, but got {opt["num_frame"]}')  #如果 opt['num_frame'] % 2 == 1是真，那么往下进行，反之，后面的输出。
...
@DATASET_REGISTRY.register()
class REDSRJPGecurrentDataset(data.Dataset):
    """REDS dataset for training recurrent networks.
    ...
    def __init__(self, opt):
        super(REDSRJPGecurrentDataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])
        self.num_frame = opt['num_frame']
...
@DATASET_REGISTRY.register()
class SigalREDSDJPGataset(data.Dataset):
    """REDS dataset for deblur training.
    ...
    def __init__(self, opt):
        super(SigalREDSDJPGataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])  # 转换成路径形式
    ...
    def __getitem__(self, index):
        ...
        if self.is_lmdb:
            ...
            img_gt_path = self.gt_root / clip_name / f'{frame_name}.png'
            img_lq_path = self.lq_root / clip_name / f'{frame_name}.jpg'
        img_bytes = self.file_client.get(img_gt_path, 'gt')
        img_gt = imfrombytes(img_bytes, float32=True)
        img_bytes = self.file_client.get(img_lq_path, 'lq')
        img_lq = imfrombytes(img_bytes, float32=True)
...
@DATASET_REGISTRY.register()
class SigalREDSDataset(data.Dataset):
    """REDS dataset for deblur training.
    ...
    def __init__(self, opt):
        super(SigalREDSDataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])  # 转换成路径形式
        self.mean = opt['mean'] if 'mean' in opt else None
        self.std = opt['std'] if 'std' in opt else None
