...
class Conv(nn.Module):
    def __init__(self, input_channels, n_feats, kernel_size, stride=1, padding=0, bias=True, bn=False, act=False):
        ...
        m = []
        m.append(nn.Conv2d(input_channels, n_feats, 1, 1, 0, bias=bias))
        m.append(nn.Conv2d(n_feats, n_feats, 3, 1, 1, groups=n_feats,  bias=bias))
...
class vivo_stage3_final_v49_DWConv(nn.Module):
    def __init__(self, scale_fact):
        super(vivo_stage3_final_v49_DWConv, self).__init__()
...
if __name__ == '__main__':
    ...
    model = vivo_stage3_final_v49_DWConv(0.75)
