...
class vivo_stage3_final_v49_bdmlp_delstage4andstage3Kelnel3To5(nn.Module):
    def __init__(self, scale_fact):
        super(vivo_stage3_final_v49_bdmlp_delstage4andstage3Kelnel3To5, self).__init__()
        self.scale_fact = scale_fact
        # self.srn1 = SRN(in_channels=3, out_channels=3, n_resblock=6, n_feat=16, kernel_size=5, ispre=False)
        self.srn2 = SRN(in_channels=3, out_channels=3, n_resblock=6, n_feat=16, kernel_size=5, ispre=False,
    ...
    def forward(self, x):
        ...
        return x_scale1_out[:, :, :H_in, :W_in], x_scale2_out[:, :, :int(H_in*0.75), :int(W_in*0.75)], x_scale4_out[:, :, :int(H_in*0.75**2), :int(W_in*0.75**2)]

if __name__ == '__main__':
    from fvcore.nn import flop_count_str, flop_count_table, FlopCountAnalysis, ActivationCountAnalysis
    model = vivo_stage3_final_v49_bdmlp_delstage4andstage3Kelnel3To5(0.75).cuda()
