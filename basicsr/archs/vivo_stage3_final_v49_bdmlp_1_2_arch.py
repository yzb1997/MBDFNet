...
class vivo_stage3_final_v49_bdmlp_1_2(nn.Module):
    def __init__(self, scale_fact):
        super(vivo_stage3_final_v49_bdmlp_1_2, self).__init__()
    ...
    def forward(self, x):
        ...
        x_scale1 = x
        x_scale2 = F.interpolate(x_scale1, scale_factor=self.scale_fact, mode='bicubic')
        x_scale4 = F.interpolate(x_scale2, scale_factor=self.scale_fact, mode='bicubic')
        ...
        x_scale4_up = F.interpolate(x_scale4_out, scale_factor=1 / self.scale_fact, mode='bilinear')

        x_scale4_out += x_scale4

        # scale2
        x_scale2_out, dnc1, dnc2 = self.srn3(x_scale2, x_scale4_up, dnc1, dnc2)
        up = F.interpolate(dnc2, scale_factor=1 / self.scale_fact, mode='bilinear')
        ...
        return x_scale1_out[:, :, :H_in, :W_in], x_scale2_out[:, :, :int(H_in*self.scale_fact), :int(W_in*self.scale_fact)], x_scale4_out[:, :, :int(H_in*self.scale_fact**2), :int(W_in*self.scale_fact**2)]

if __name__ == '__main__':
    from thop import profile
    import torch

    model = vivo_stage3_final_v49_bdmlp_1_2(0.5).cuda()

    input_dim = torch.randn(1, 3, 256, 256).cuda()
    flops, params = profile(model=model, inputs=(input_dim,))

    # print(model.IdynamicFlops())
    ...
    # model = vivo_stage3_final_v49_bdmlp_mlpblock(0.75).cuda()
    #
