...
class convRes2Net(nn.Module):
    def __init__(self, input_channels, n_feats, kernel_size, stride=1, padding=0, bias=True, bn=False, act=False):
        super(convRes2Net, self).__init__()

        self.scale = 4
        convs = []
        for i in range(4):
          convs.append(nn.Conv2d(input_channels//4, n_feats//4, kernel_size=kernel_size, stride = stride, padding=padding, bias=bias))
        self.convs = nn.ModuleList(convs)
        self.pointWiseConv = nn.Conv2d(input_channels, input_channels, 1, 1, 0)
    ...
    def forward(self, input):
        # res = input
        spx = torch.split(input, self.input_channels//4, 1)
        for i in range(self.num):
            if i == 0:
                sp = spx[i]
            else:
                sp = sp * spx[i]
            sp = F.gelu(self.convs[i](sp))
            if i == 0:
                out = sp
            else:
                out = torch.cat([out, sp], dim=1)
        # out = torch.cat((out, spx[self.num]), 1)
...
class ResBlock(nn.Module):
    def __init__(self, n_feat, kernel_size, padding=0, bias=True, bn=False, act=nn.ReLU(True), res_scale=1):
        ...
        m = []
        for i in range(1):
            m.append(convRes2Net(n_feat, n_feat, kernel_size, padding=padding, bias=bias))
...
class vivo_stage3_final_v49_res2net(nn.Module):
    def __init__(self, scale_fact):
        super(vivo_stage3_final_v49_res2net, self).__init__()
...
if __name__ == '__main__':
    model = vivo_stage3_final_v49_res2net(0.75).cuda()
