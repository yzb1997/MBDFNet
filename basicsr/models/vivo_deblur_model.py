...
class vivo_scale4_Model(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):    #初始化SRModel类，比如定义网络和load weight
        super(vivo_scale4_Model, self).__init__(opt)
...
# '''
    ...
    def optimize_parameters(self, current_iter):    #优化参数，即一个完整的train的step，包括了forward，loss的计算，backward，参数优化等
        ...
        self.gt5 = F.interpolate(self.gt4, scale_factor=0.5, mode='bicubic')

        self.output1 = self.output[0] # 1/4
        self.output2 = self.output[1] # 1/8
        self.output3 = self.output[2] # 1/16
        ...
        if self.l1_pix:
            l_l1_1 = self.l1_pix(self.output4, self.gt1)
            l_l1_2 = self.l1_pix(self.output1, self.gt3)
            l_l1_3 = self.l1_pix(self.output2, self.gt4)
            l_l1_4 = self.l1_pix(self.output3, self.gt5)
        ...
        if self.fq_pix:
            l_fq_1 = self.fq_pix(self.output4, self.gt1)
            l_fq_2 = self.fq_pix(self.output1, self.gt3)
            l_fq_3 = self.fq_pix(self.output2, self.gt4)
            l_fq_4 = self.fq_pix(self.output3, self.gt5)
        ...
        if self.ssim_loss:
            l_ssim_1 = self.ssim_loss(self.output4, self.gt1)
            l_ssim_2 = self.ssim_loss(self.output1, self.gt3)
            l_ssim_3 = self.ssim_loss(self.output2, self.gt4)
            l_ssim_4 = self.ssim_loss(self.output3, self.gt5)
            l_total_ssim = l_total_ssim + l_ssim_1 + l_ssim_2 + l_ssim_3 + l_ssim_4
    ...
    def get_current_visuals(self):  #得到网络的输出的结果，这个函数会在validation中用到（实际可以简化）
        ...
        if type(self.output) == tuple:
            out_dict['result'] = self.output[3].detach().cpu()
