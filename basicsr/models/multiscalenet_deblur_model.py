...
# '''
    ...
    def optimize_parameters(self, current_iter):    #优化参数，即一个完整的train的step，包括了forward，loss的计算，backward，参数优化等
        ...
        l_total = l_total + l_total_l1 + l_total_ct + l_total_hem + l_total_tv + l_total_gr + l_total_bd + l_total_ssim + l_total_fq
    ...
    def get_current_visuals(self):  #得到网络的输出的结果，这个函数会在validation中用到（实际可以简化）
        ...
        if type(self.output) == tuple:
            # self.input1 = self.lq
            # self.input2 = F.interpolate(self.input1, scale_factor=0.5, mode='bicubic') # 1/2
            # self.input3 = F.interpolate(self.input2, scale_factor=0.5, mode='bicubic') # 1/4
            # self.input4 = F.interpolate(self.input3, scale_factor=0.5, mode='bicubic') # 1/8
            # self.input5 = F.interpolate(self.input4, scale_factor=0.5, mode='bicubic') # 1/16
...
@MODEL_REGISTRY.register()
class deeprft_Model(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):    #初始化SRModel类，比如定义网络和load weight
        super(deeprft_Model, self).__init__(opt)
...
# '''
    ...
    def optimize_parameters(self, current_iter):    #优化参数，即一个完整的train的step，包括了forward，loss的计算，backward，参数优化等
        ...
        loss_dict = OrderedDict()
        if self.msc_pix:
            l_msc_1 = self.msc_pix(self.output1, self.gt1)
            l_msc_2 = self.msc_pix(self.output2, self.gt2)
            l_msc_3 = self.msc_pix(self.output3, self.gt3)
            l_total_msc = l_total_msc + l_msc_1 + l_msc_2 + l_msc_3
            loss_dict['l_msc_1'] = l_msc_1
            loss_dict['l_msc_2'] = l_msc_2
            loss_dict['l_msc_3'] = l_msc_3
            loss_dict['l_total_msc'] = l_total_msc

        if self.msed_pix:
            l_msed_1 = self.msed_pix(self.output1, self.gt1)
            l_msed_2 = self.msed_pix(self.output2, self.gt2)
            l_msed_3 = self.msed_pix(self.output3, self.gt3)
            l_total_msed = l_total_msed + l_msed_1 + l_msed_2 + l_msed_3
            loss_dict['l_msed_1'] = l_msed_1
            loss_dict['l_msed_2'] = l_msed_2
            loss_dict['l_msed_3'] = l_msed_3
            loss_dict['l_total_msed'] = l_total_msed

        if self.msfr_pix:
            l_msfr_1 = self.msfr_pix(self.output1, self.gt1)
            l_msfr_2 = self.msfr_pix(self.output2, self.gt2)
            l_msfr_3 = self.msfr_pix(self.output3, self.gt3)
            l_total_msfr = l_total_msfr + l_msfr_1 + l_msfr_2 + l_msfr_3
            loss_dict['l_msfr_1'] = l_msfr_1
            loss_dict['l_msfr_2'] = l_msfr_2
            loss_dict['l_msfr_3'] = l_msfr_3
            loss_dict['l_total_msfr'] = l_total_msfr


        l_total = l_total + l_total_msc + l_total_msed + l_total_msfr
        loss_dict['l_total'] = l_total
        l_total.backward()
        self.optimizer_g.step()
...
# '''
    ...
    def optimize_parameters(self, current_iter):    #优化参数，即一个完整的train的step，包括了forward，loss的计算，backward，参数优化等
        ...
        l_total = 0
        l_total_cri = 0
        l_total_perceptual = 0
        l_total_gan = 0
        loss_dict = OrderedDict()
        if self.cri_pix:
            l_cri_1 = self.cri_pix(self.output1, self.gt1)
            l_cri_2 = self.cri_pix(self.output2, self.gt2)
            l_cri_3 = self.cri_pix(self.output3, self.gt3)
            l_total_cri = l_total_cri + l_cri_1 + l_cri_2 + l_cri_3
            loss_dict['l_cri_1'] = l_cri_1
            loss_dict['l_cri_2'] = l_cri_2
            loss_dict['l_cri_3'] = l_cri_3
            loss_dict['l_total_cri'] = l_total_cri

        if self.cri_perceptual:
            l_g_percep_1, l_g_style_1 = self.cri_perceptual(self.output1, self.gt1)
            l_g_percep_2, l_g_style_2 = self.cri_perceptual(self.output2, self.gt2)
            l_g_percep_3, l_g_style_3 = self.cri_perceptual(self.output3, self.gt3)
            if l_g_percep_1 is not None:
                l_total_perceptual += l_g_percep_1
            if l_g_percep_2 is not None:
                l_total_perceptual += l_g_percep_2
            if l_g_percep_3 is not None:
                l_total_perceptual += l_g_percep_3
            if l_g_style_1 is not None:
                l_total_perceptual += l_g_style_1
            if l_g_style_2 is not None:
                l_total_perceptual += l_g_style_2
            if l_g_style_3 is not None:
                l_total_perceptual += l_g_style_3
            loss_dict['l_total_perceptual'] = l_total_perceptual

        if self.cri_gan:
            l_total_gan_1 = self.cri_gan(self.output1, self.gt1)
            l_total_gan_2 = self.cri_gan(self.output2, self.gt2)
            l_total_gan_3 = self.cri_gan(self.output3, self.gt3)
            l_total_gan = l_total_gan + l_total_gan_1 + l_total_gan_2 + l_total_gan_3
            loss_dict['l_total_gan_1'] = l_total_gan_1
            loss_dict['l_total_gan_2'] = l_total_gan_2
            loss_dict['l_total_gan_3'] = l_total_gan_3
            loss_dict['l_total_gan'] = l_total_gan

        l_total = l_total + l_total_cri + l_total_perceptual
        loss_dict['l_total'] = l_total
