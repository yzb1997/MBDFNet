...
class mimo_pan_Model(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):    #初始化SRModel类，比如定义网络和load weight
        super(mimo_pan_Model, self).__init__(opt)
...
# '''
    ...
    def optimize_parameters(self, current_iter):    #优化参数，即一个完整的train的step，包括了forward，loss的计算，backward，参数优化等
        ...
        self.gt1 = self.gt
        self.gt2 = F.interpolate(self.gt1, scale_factor=0.8, mode='bicubic')
...
@MODEL_REGISTRY.register()
class mimo_pan_Model_075(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):    #初始化SRModel类，比如定义网络和load weight
        super(mimo_pan_Model_075, self).__init__(opt)
...
@MODEL_REGISTRY.register()
class mimo_pan_Model_075_s4_svlrm(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):    #初始化SRModel类，比如定义网络和load weight
        super(mimo_pan_Model_075_s4_svlrm, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):    #优化参数，即一个完整的train的step，包括了forward，loss的计算，backward，参数优化等
        ...
        loss_dict = OrderedDict()
        if self.l1_pix:
            l_l1_1 = self.l1_pix(self.output1, self.
                                 gt1)
            l_l1_2 = self.l1_pix(self.output2, self.gt2)
            l_l1_3 = self.l1_pix(self.output3, self.gt3)
            l_l1_4 = self.l1_pix(self.output4, self.gt4)
            l_l1_5 = self.l1_pix(self.output5, self.gt)
            l_total_l1 = l_total_l1 + l_l1_1 + l_l1_2 + l_l1_3 + l_l1_4 + l_l1_5
            loss_dict['l_l1_1'] = l_l1_1
            loss_dict['l_l1_2'] = l_l1_2
            loss_dict['l_l1_3'] = l_l1_3
            loss_dict['l_l1_4'] = l_l1_4
        ...
        if self.fq_pix:
            ...
            l_fq_3 = self.fq_pix(self.output3, self.gt3)
            l_fq_4 = self.fq_pix(self.output4, self.gt4)
            l_fq_5 = self.fq_pix(self.output5, self.gt)
            l_total_fq = l_total_fq + l_fq_1 + l_fq_2 + l_fq_3 + l_fq_4 + l_fq_5
            loss_dict['l_fq_1'] = l_fq_1
            loss_dict['l_fq_2'] = l_fq_2
            loss_dict['l_fq_3'] = l_fq_3
            loss_dict['l_fq_4'] = l_fq_4
    ...
    def get_current_visuals(self):  #得到网络的输出的结果，这个函数会在validation中用到（实际可以简化）
        ...
        if type(self.output) == tuple:
            out_dict['result'] = self.output[4].detach().cpu()
        else:
            out_dict['result'] = self.output.detach().cpu()
        if hasattr(self, 'gt'):
            out_dict['gt'] = self.gt.detach().cpu()
