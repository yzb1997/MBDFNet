...
class MSN_SRModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):    #初始化SRModel类，比如定义网络和load weight
        super(MSN_SRModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):    #优化参数，即一个完整的train的step，包括了forward，loss的计算，backward，参数优化等
        ...
        self.gt1 = self.gt
        self.gt2 = F.interpolate(self.gt, scale_factor=0.5, mode='bilinear')
        self.gt3 = F.interpolate(self.gt, scale_factor=0.25, mode='bilinear')
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):    ##
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # print(f'visuals[result]_size():', visuals['result'].size())
            sr_img = tensor2img([visuals['result']])
            metric_data['img'] = sr_img
            # print(f'metric_data[img]_size():', metric_data['img'].shape)
            if 'gt' in visuals:
    ...
    def get_current_visuals(self):  #得到网络的输出的结果，这个函数会在validation中用到（实际可以简化）
        ...
        if hasattr(self, 'gt'):
            ...
            # print(f'gt_img:', out_dict['gt'].shape)
        return out_dict
